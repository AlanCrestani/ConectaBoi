...
const generateHistoricoData = () => {
  ...
  for (let i = 7; i >= 1; i--) {
    ...
    historico.push({
      ...
      leituraNoturna: ["verde", "amarelo", "vermelho"][
...
const LeituraCocho = () => {
  ...
  const [dadosGraficoCompletos, setDadosGraficoCompletos] = useState<
    DadosGrafico[]
  >([]); // Dados completos para paginação
  ...
  const fetchDadosNivel1 = async () => {
    ...
    try {
      ...
      const { data: fatoTrato, error } = await supabase
        .from("fato_trato")
        .select("previsto_kg, realizado_kg")
  ...
  const fetchDadosNivel2 = async () => {
    ...
    try {
      const { data: fatoTrato, error } = await supabase
        .from("fato_trato")
        .select("previsto_kg, realizado_kg, tipo_dieta")
        .eq("data", dataSelecionada)
        .not("tipo_dieta", "is", null);
      if (error) {
        console.error("Erro ao buscar dados nível 2:", error);
        return;
      }
      if (fatoTrato && fatoTrato.length > 0) {
        const agrupado = fatoTrato.reduce(
          (acc, item) => {
        ...
        const dadosFormatados = ordemTipos
          .filter((tipo) => agrupado[tipo])
          .map((tipo) => {
  ...
  const fetchDadosNivel3 = async (dietaSelecionada: string) => {
    setIsLoading(true);
    try {
      const { data: fatoTrato, error: errorTrato } = await supabase
        .from("fato_trato")
        .select("previsto_kg, realizado_kg, id_curral")
        .eq("data", dataSelecionada)
        .eq("tipo_dieta", dietaSelecionada)
        .not("id_curral", "is", null);
      if (errorTrato) {
        console.error("Erro ao buscar dados nível 3:", errorTrato);
        return;
      }
      if (fatoTrato && fatoTrato.length > 0) {
        const agrupadoPorCurral = fatoTrato.reduce(
          (acc, item) => {
        ...
        const { data: fatoResumo, error: errorResumo } = await supabase
          .from("fato_resumo")
          .select("id_curral, curral")
          .eq("data", dataSelecionada)
          .in("id_curral", Object.keys(agrupadoPorCurral));
        if (errorResumo) {
          console.error("Erro ao buscar nomes dos currais:", errorResumo);
          return;
        }
        const mapCurrais =
          fatoResumo?.reduce((acc, item) => {
            acc[item.id_curral] = item.curral;
            return acc;
          }, {} as Record<string, string>) || {};
        const dadosFormatados = Object.entries(agrupadoPorCurral)
          .map(([idCurral, dados]) => ({
        ...
        const dadosPaginados = dadosFormatados.slice(
          inicio,
  ...
  const fetchDadosNivel4 = async (curralSelecionado: string) => {
    ...
    try {
      ...
      const { data: fatoResumo, error } = await supabase
        .from("fato_resumo")
        .select("data, cms_previsto_kg, cms_realizado_kg")
  ...
  const fetchDadosNivel5 = async (
    ...
    try {
      ...
      const { data: fatoTrato, error } = await supabase
        .from("fato_trato")
        .select("previsto_kg, realizado_kg, trato, id_carregamento")
        .eq("id_curral", curralSelecionado)
        .eq("data", dataHistorico)
        .order("trato", {
          ascending: true,
        });
  ...
  const fetchDadosNivel6 = async (
    ...
    try {
      ...
      const { data: fatoCarregamento, error } = await supabase
        .from("fato_carregamento")
        .select("previsto_kg, realizado_kg, ingrediente")
        .eq("id_carregamento", idCarregamento)
        .order("ingrediente", {
          ascending: true,
        });
      ...
      if (fatoCarregamento && fatoCarregamento.length > 0) {
        ...
        const agrupadoPorIngrediente = fatoCarregamento.reduce(
          (acc, item) => {
        ...
        const dadosFormatados = Object.entries(agrupadoPorIngrediente).map(
          ([ingrediente, dados]) => ({
            nome: ingrediente,
            previsto: dados.previsto,
            realizado: dados.realizado,
  ...
  const fetchDadosCurralSelecionado = async (curralSelecionado: string) => {
    try {
      ...
      if (fatoResumo) {
        ...
        const mediaConsumo4Dias =
          historico4Dias && historico4Dias.length > 0
            ? historico4Dias.reduce(
                (acc, item) => acc + (item.cms_realizado_kg || 0),
                0
              ) / historico4Dias.length
            : 0;
    ...
    switch (drillDown.nivel) {
      ...
      case 5:
        if (drillDown.curralSelecionado && drillDown.dataSelecionadaHistorico) {
          fetchDadosNivel5(
            drillDown.curralSelecionado,
            drillDown.dataSelecionadaHistorico
          );
        }
        break;
      case 6:
        if (
          drillDown.curralSelecionado &&
          drillDown.dataSelecionadaHistorico &&
          drillDown.tratoSelecionado
        ) {
          // Buscar o id_carregamento do trato selecionado através da consulta
          const fetchIdCarregamento = async () => {
            const { data: tratoData } = await supabase
              .from("fato_trato")
              .select("id_carregamento")
              .eq("id_curral", drillDown.curralSelecionado)
              .eq("data", drillDown.dataSelecionadaHistorico)
              .eq("trato", drillDown.tratoSelecionado)
              .single();
  ...
  const getBreadcrumb = () => {
    ...
    if (drillDown.nivel >= 2) {
      if (drillDown.dietaSelecionada) {
        breadcrumbs.push(
          `Dieta ${drillDown.dietaSelecionada
            .toLowerCase()
    ...
    if (drillDown.nivel >= 4) {
      const nomeCurral =
        dadosGraficoCompletos.find(
          (c) => c.id_curral === drillDown.curralSelecionado
        )?.nome || "Curral";
  ...
  const handleBarClick = (
    data: any,
    barraClicada: "previsto" | "realizado"
  ) => {
  ...
  const handleBreadcrumbClick = (indice: number) => {
    ...
    if (indice <= 1) {
      ...
      indice === 4 ||
      (indice === 5 && breadcrumbs[indice] === "Histórico de consumo")
    ) {
    ...
    } else if (breadcrumbs[indice] === "Desvio de trato") {
      // Desvio de trato - vai para nível 5
      if (
        drillDown.dietaSelecionada &&
        drillDown.curralSelecionado &&
        drillDown.dataSelecionadaHistorico
    ...
    } else if (breadcrumbs[indice] === "Desvio de carregamento") {
      // Desvio de carregamento - vai para nível 6
      if (
        drillDown.dietaSelecionada &&
        drillDown.curralSelecionado &&
        drillDown.dataSelecionadaHistorico &&
        ...
        setDrillDown({
          ...
          curralSelecionado: drillDown.curralSelecionado,
          dataSelecionadaHistorico: drillDown.dataSelecionadaHistorico,
          tratoSelecionado: drillDown.tratoSelecionado,
        });
      }
    }
    setPaginaAtual(0);
  ...
  const handleNotaClick = (nota: number) => {
    ...
    novoHistorico[7].previsto =
      (novoHistorico[6]?.realizado || 8.2) +
      (ajusteValues[nota.toString()] || 0);
  ...
  const handleSalvarLeitura = () => {
    ...
    if (
      ...
      curralAtual < curraisDisponiveis.length - 1
    ) {
  ...
  const getTituloGrafico = () => {
    switch (drillDown.nivel) {
      ...
      case 5:
        ...
        const dataFormatada = drillDown.dataSelecionadaHistorico
          ? new Date(drillDown.dataSelecionadaHistorico).toLocaleDateString(
              "pt-BR"
            )
          : "Data";
  ...
  const proximaPagina = () =>
    setPaginaAtual(Math.min(totalPaginas - 1, paginaAtual + 1));
  return (
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Sistema inteligente para lançamento de leituras
          </p>
        </div>
        ...
        <div className="flex items-center gap-4">
              ...
              <SelectContent>
                {Array.from(
                  ...
                  (_, i) => {
                    ...
                    return (
                      <SelectItem key={dataStr} value={dataStr}>
          ...
          <div className="flex items-center space-x-3">
            <div className="flex items-center space-x-1">
              <Label htmlFor="curva-meta" className="text-xs">
                Meta
              </Label>
              <Switch
                id="curva-meta"
                checked={configuracoes.curvaMeta}
            ...
            <div className="flex items-center space-x-1">
              <Label htmlFor="curva-media" className="text-xs">
                Média
              </Label>
              <Switch
                id="curva-media"
                checked={configuracoes.curvaMedia}
                onCheckedChange={(checked) =>
                  setConfiguracoes({
                    ...configuracoes,
                    curvaMedia: checked,
        ...
        <div className="grid grid-cols-4 gap-3 mb-4">
          ...
          <Card>
            <CardContent className="p-3">
              <div className="text-center">
                <div className="text-lg font-bold">
                  {dadosCurralSelecionado?.cabecas ?? loteInfo.cabecas}
                </div>
          ...
          <Card>
            <CardContent className="p-3">
              <div className="text-center">
                <div className="text-lg font-bold">
                  {dadosCurralSelecionado?.diasCocho ?? loteInfo.diasCocho}
                </div>
          ...
          <Card>
            <CardContent className="p-3">
              <div className="text-center">
                <div className="text-lg font-bold">
                  {dadosCurralSelecionado?.consumoMedio4Dias ??
                    loteInfo.consumoMedio4Dias}
                </div>
            ...
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-3">
                  ...
                  {drillDown.nivel === 3 && totalPaginas > 1 && (
                  ...
                  {drillDown.nivel === 4 &&
                    dadosGraficoCompletos.length > 0 && (
                      <div className="flex items-center space-x-2">
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => {
                            const curralIndex = dadosGraficoCompletos.findIndex(
                              (c) => c.id_curral === drillDown.curralSelecionado
                            );
                            if (curralIndex > 0) {
                              setDrillDown({
                                ...drillDown,
                                curralSelecionado:
                                  dadosGraficoCompletos[curralIndex - 1]
                                    .id_curral,
                              });
                            }
                          }}
                          disabled={
                            dadosGraficoCompletos.findIndex(
                              (c) => c.id_curral === drillDown.curralSelecionado
                            ) === 0
                          }
                        >
                        ...
                        <span className="text-sm font-medium px-2">
                          ...
                          <span className="text-xs text-muted-foreground ml-1">
                            ...
                            de {dadosGraficoCompletos.length})
                          </span>
                        </span>
                        <Button
                          variant="outline"
                          ...
                          onClick={() => {
                            ...
                            if (
                              curralIndex <
                              dadosGraficoCompletos.length - 1
                            ) {
                              setDrillDown({
                                ...drillDown,
                                curralSelecionado:
            ...
            <CardContent>
              {isLoading ? (
                ...
                <ResponsiveContainer width="100%" height={300}>
                  {drillDown.nivel === 4 && configuracoes.curvaMedia ? (
                    <ComposedChart
                      data={dadosGrafico}
                      margin={{
                        top: 20,
                    ...
                    >
                      ...
                      <Bar
                        ...
                        fill="#3b82f6"
                        onClick={(data, index) =>
                          handleBarClick(dadosGrafico[index], "previsto")
                        }
                        style={{
                          cursor: "pointer",
                        }}
                      ...
                      <Bar
                        dataKey="realizado"
                        onClick={(data, index) =>
                          handleBarClick(dadosGrafico[index], "realizado")
                        }
                        style={{
                          cursor: "pointer",
                        }}
                      >
                        {dadosGrafico.map((entry, index) => (
                          <Cell
                            key={`cell-${index}`}
                            fill={getBarColor(entry, "realizado")}
                          />
                        ))}
                      </Bar>
                      <Line
                ...
                <div className="mt-4 p-3 bg-muted rounded-lg">
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    ...
                    <div>
                      ...
                      <p className="font-semibold">
                        {dadosGrafico
                          .reduce((acc, item) => acc + item.realizado, 0)
                          .toLocaleString("pt-BR", {
                            minimumFractionDigits: 2,
                          })}{" "}
                        kg
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <div className="grid grid-cols-4 gap-2">
                      {notasLeitura.map((item) => (
                        <Button
                          key={item.nota}
                          variant={
                            leituraCocho === item.nota ? "default" : "outline"
                          }
                          size="sm"
                          onClick={() => handleNotaClick(item.nota)}
