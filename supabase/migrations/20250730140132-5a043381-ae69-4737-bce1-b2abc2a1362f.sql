...
-- Função para verificar se usuário tem role específica em confinamento
CREATE OR REPLACE FUNCTION public.user_has_role_in_confinamento(confinamento_id UUID, required_role user_role)
RETURNS BOOLEAN
LANGUAGE plpgsql
STABLE SECURITY DEFINER
SET search_path = ''
...
BEGIN
  RETURN EXISTS (
    ...
    JOIN public.user_confinamentos uc ON ur.user_id = uc.user_id AND ur.confinamento_id = uc.confinamento_id
    WHERE uc.confinamento_id = $1 
...
AS $$
BEGIN
  INSERT INTO public.profiles (user_id, nome_completo, funcao)
  VALUES (
    NEW.id, 
