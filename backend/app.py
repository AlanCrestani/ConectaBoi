...
def start_server():
    """Iniciar servidor start_server.py"""
    try:
        import subprocess
        import os
        
        # Caminho para o script start_server.py
        ...
        if not os.path.exists(script_path):
            return jsonify({
                'success': False, 
                'message': f'Arquivo start_server.py não encontrado em {script_path}'
            }), 404
        ...
        return jsonify({
            'success': True,
            'message': 'Servidor start_server.py iniciado com sucesso',
            'pid': process.pid
        })
